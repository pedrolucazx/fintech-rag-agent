...
test("consults invoices and preserves the tool exchange for the next turn", async () => {
  for (const id of ["fat_202609", "fat_000000"]) {
    ...
    await runHarness(
      ...
      async (messages) => {
        assert.equal(
          messages.find((message) => message.role === "tool")?.toolCall?.args
            .id,
          id,
        );
  ...
  assert.equal(
    await runHarness(
      ...
      async (messages) => {
        ...
        return {
          type: "text",
          content: JSON.parse(messages.at(-1)!.content).status,
        };
      },
      mockRetrieve,
    ),
...
test("invalid tool arguments return an error to the model without stopping the conversation", async () => {
  ...
  const result = await runHarness(
    "invoice-invalid-id",
    "Minha fatura foi paga?",
    async (messages) => {
      if (++calls === 1)
        return { type: "tool_call", name: "consultar_status_fatura", args: {} };
...
test("LLM adapter sends the assistant tool call and matching result over the SDK transport", async (t) => {
  ...
  t.mock.method(
    ...
    async (_input: unknown, init: RequestInit) => {
      ...
      return Response.json({
        choices: [
          { message: { role: "assistant", content: "Sua fatura está paga." } },
        ],
      });
    },
  );
  assert.equal(
    await runHarness(
      "invoice-sdk",
      `Minha fatura fat_202609 já foi paga? ${crypto.randomUUID()}`,
      undefined,
      mockRetrieve,
    ),
