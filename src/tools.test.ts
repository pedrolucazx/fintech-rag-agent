...
test("appends tickets with the conversation id and returns its identifier", async () => {
  ...
  try {
    const [first, second] = await Promise.all([
      abrirTicket(
        {
          ...
          description: "A fatura fat_202609 foi debitada duas vezes.",
        },
        "chat-test",
        ticketsPath,
      ),
      abrirTicket(
        {
          subject: "Boleto indisponível",
          description: "A segunda via não aparece no aplicativo.",
        },
        "chat-test",
        ticketsPath,
      ),
    ]);
    const tickets = JSON.parse(await readFile(ticketsPath, "utf8")) as Array<
      Record<string, unknown>
    >;

    assert.equal(tickets.length, 2);
    assert.deepEqual(
      tickets.map(({ id, createdAt }) => ({ id, createdAt })),
      [first, second],
    );
    assert.equal(tickets[0].subject, "Cobrança duplicada");
    assert.equal(
      tickets[0].description,
      "A fatura fat_202609 foi debitada duas vezes.",
    );
...
test("rejects tickets without a meaningful subject or description", async () => {
  await assert.rejects(
    abrirTicket({ subject: "", description: "detalhes" }),
    /assunto/i,
  );
  await assert.rejects(
    abrirTicket({ subject: "Assunto", description: " " }),
    /descrição/i,
  );
});
...
test("failed ticket write preserves existing tickets and the queue recovers", async (t) => {
  ...
  const failure = t.mock.method(
    fs,
    "writeFile",
    async (path: Parameters<typeof fs.writeFile>[0]) => {
  ...
  try {
    await assert.rejects(
      abrirTicket(
        { subject: "Assunto", description: "Detalhes" },
        "chat-test",
        ticketsPath,
      ),
